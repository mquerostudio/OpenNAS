...
def test_OpenNASFW(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
...
def test_OpenNASFW_linux(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
@pytest.mark.macos
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_OpenNASFW_macos(dut: IdfDut) -> None:
    dut.expect('Hello world!')
...
def test_OpenNASFW_host(app: QemuApp, dut: QemuDut) -> None:
